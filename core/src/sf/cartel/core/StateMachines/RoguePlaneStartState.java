...
public class RoguePlaneStartState extends State<RoguePlaneStateMachine> {
    private AnimationController animationController;
    private float deltaAccumulator;

    public RoguePlaneStartState(RoguePlaneStateMachine stateMachine, AnimationController animationController) {
        super(stateMachine);
    ...
    public void update(float delta) {
        ...
        if(deltaAccumulator <= 1.5f)
            stateMachine.getGameObject().getSprite().setScale(GoodMath.map(deltaAccumulator, 0f, 1.5f, 0, stateMachine.scaleMin));
        if(deltaAccumulator >= 3f) {
            stateMachine.transition(StateMachineStates.RoguePlaneTravel.ordinal());
        }
